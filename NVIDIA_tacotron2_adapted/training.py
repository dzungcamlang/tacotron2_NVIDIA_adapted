...
def load_model(hyper_params):
    ...
    if hyper_params['distributed_run']:
        # the model already has a special method to apply gradient by allreduce
...
def validate(model, criterion, val_loader, iteration, n_gpus, logger, distributed_run, rank):
...
def train(output_directory, log_directory, checkpoint_path, warm_start, n_gpus, rank, group_name,
          hyper_params, train_loader, val_loader):
    ...
    num_trainable_parameters = count_parameters(model=model)
    print("Number of trainable parameters: {}".format(num_trainable_parameters))
    ...
    for epoch in range(epoch_offset, hyper_params['epochs']):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if hyper_params['distributed_run']:
                # Computes the mean of the loss by inter-GPU communication with AllReduce
            ...
            if not overflow and (iteration % hyper_params['iters_per_checkpoint'] == 0):
                validate(model, criterion, val_loader, iteration, n_gpus, logger, hyper_params['distributed_run'], rank)
                if rank == 0:
